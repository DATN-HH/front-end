...
export function getIconByName(iconName: string): IconComponent {
  ...
  if (LucideIcons[normalizedName as keyof typeof LucideIcons]) {
    return LucideIcons[normalizedName as keyof typeof LucideIcons] as IconComponent;
  }
  ...
  const variations = [
    ...
    iconName.replace(/[-_]/g, '').charAt(0).toUpperCase() + iconName.replace(/[-_]/g, '').slice(1)
  ];
  
  for (const variation of variations) {
    if (LucideIcons[variation as keyof typeof LucideIcons]) {
      return LucideIcons[variation as keyof typeof LucideIcons] as IconComponent;
    }
  }
