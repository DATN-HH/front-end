...
import { Star, MessageSquare, Plus, Send, Calendar, MapPin, User } from 'lucide-react';
...
const StarRatingOnly = ({ rating, size = 16 }: { rating: number; size?: number }) => {
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
...
const ReviewCard = ({ review, onResponse }: { review: Review; onResponse: (reviewId: number, response: string) => void }) => {
          ...
          <div className="flex-1">
            ...
            <div className="flex items-start justify-between mb-2">
              ...
              <div className="flex gap-2">
                <Badge variant={review.feedbackStatus === 'RESPONDED' ? 'default' : 'secondary'}>
                  {review.feedbackStatus}
                </Badge>
                <Badge variant="outline" className={review.feedbackType === 'RESTAURANT' ? 'border-blue-200 text-blue-700 bg-blue-50' : 'border-green-200 text-green-700 bg-green-50'}>
                  {review.feedbackType === 'RESTAURANT' ? '🏪 Restaurant' : '🍽️ Product'}
                </Badge>
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(review.categoryRatings).map(([category, rating]) => (
                    <div key={category} className="flex items-center justify-between text-sm">
                      ...
                      <StarRatingOnly rating={rating} size={14} />
                    </div>
                  ))}
                </div>
              </div>
            ...
            {review.responseText && (
              <div className="mt-4 p-4 bg-blue-50 rounded-lg border-l-4 border-blue-400">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-blue-900">Management Response</span>
                  {review.responseDate && (
                ...
                <p className="text-blue-800">{review.responseText}</p>
              </div>
            )}
...
export default function FeedbackPage() {
  ...
  const handleResponse = async (reviewId: number, responseText: string) => {
    try {
      await managerFeedbackAPI.respondToFeedback(reviewId, { responseText });
      toast.success('Success', 'Response sent successfully!');
      refetch();
    } catch (error: any) {
      console.error('Failed to send response:', error);
      toast.error('Error', error.response?.data?.message || 'Failed to send response');
    }
  };
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
              ...
              <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Share your experience with us. Your feedback helps us improve our service.
                  </DialogDescription>
                </DialogHeader>
                
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                  <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="restaurant">Restaurant Review</TabsTrigger>
                    <TabsTrigger value="product">Product Review</TabsTrigger>
                  </TabsList>
