...
export default function TableBookingPage() {
  ...
  const handleTableSelect = useCallback((tables: TableResponse[]) => {
    ...
    setBookingData((prev) => ({
      ...
      tableIds,
      guests:
        maxCapacity > 0 ? Math.min(prev.guests, maxCapacity) : prev.guests,
    }));
  }, []);
          ...
          <Card>
                    ...
                    <div className="mt-2 pt-2 border-t border-blue-200">
                      ...
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-semibold text-green-600">
                          {formatCurrency(
                            selectedTables.reduce(
                              (sum, t) => sum + t.tableType.depositForBooking,
                              0
                            )
                          )}
                        </span>
            ...
            <CardContent>
              ...
              {selectedFloor && floorData && (
                <div className="bg-gray-50 rounded-lg p-3">{floorCanvas}</div>
              )}
            </CardContent>
          </Card>
        </div>
