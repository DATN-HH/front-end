...
import { Calendar, Clock, Users, CreditCard, Timer, Utensils } from "lucide-react"
import { CreateBookingResponse, useCheckBookingPaymentStatus } from "@/api/v1/table-booking"
...
export function BookingConfirmDialog({
  ...
  const { data: paymentStatusData, refetch: refetchPaymentStatus } = useCheckBookingPaymentStatus(
    ...
    try {
      ...
      if (result.data?.success && result.data.payload) {
        ...
        if (paymentStatus.bookingStatus === 'DEPOSIT_PAID') {
          ...
          error("Booking Expired", "This booking has expired. Please make a new reservation.")
        } else if (paymentStatus.paymentRequired) {
          error("Payment Required", `${paymentStatus.statusMessage}. Please complete payment first.`)
        } else {
          error("Cannot Confirm", paymentStatus.statusMessage || "Booking cannot be confirmed at this time.")
        }
      } else {
        error("Error", "Failed to check booking status. Please try again.")
      }
    } catch (err: any) {
      console.error("Error checking booking payment status:", err)
      // Handle specific error codes from API
      if (err.response?.status === 410) {
        error("Booking Expired", "This booking has expired and payment is no longer possible. Please make a new reservation.")
      } else if (err.response?.status === 404) {
        error("Booking Not Found", "Booking not found. Please check your booking ID.")
      } else {
        error("Error", "Failed to check booking status. Please try again.")
      }
    } finally {
  ...
  return (
    <>
              ...
              <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        {paymentStatusData?.success && paymentStatusData.payload && (
                          <p className="text-xs text-gray-500 mt-1">
                            {paymentStatusData.payload.minutesUntilExpiry > 0
                              ? `${paymentStatusData.payload.minutesUntilExpiry} minutes left`
                              : paymentStatusData.payload.expired ? 'Expired' : 'No time limit'
                            }
                          </p>
                        )}
                      </div>
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  {onConfirm && (
                    ...
                    >
                      {isCheckingStatus ? "Checking..." : "Confirm"}
                    </Button>
                  )}
                </div>
              </>
      ...
      <Dialog open={showMenuSuggestion} onOpenChange={setShowMenuSuggestion}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">Your table is booked!</h3>
              <p className="text-gray-600 mb-4">
                Would you like to pre-order food for your reservation?
              </p>
              <p className="text-sm text-gray-500">
                Skip the wait and have your meal ready when you arrive.
              </p>
            </div>
