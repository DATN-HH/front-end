...
function RestaurantDashboard() {
    ...
    const prevApiParams: SalesStatisticsRequest = useMemo(() => {
        ...
        const prevStartDate = format(
            subDays(new Date(startDate), daysDiff + 1),
            'yyyy-MM-dd'
        );
    ...
    } = useSalesStatistics(apiParams);
    const {
        data: orderMetrics,
        isLoading: orderLoading,
        refetch: refetchOrders,
    } = useOrderMetrics(apiParams);
    const {
        data: orderStatus,
        isLoading: statusLoading,
        refetch: refetchStatus,
    } = useOrderStatusStatistics(apiParams);
    const {
        data: peakHours,
        isLoading: peakLoading,
        refetch: refetchPeak,
    } = usePeakHoursStatistics(apiParams);
    const {
        data: productStats,
        isLoading: productLoading,
        refetch: refetchProducts,
    } = useProductStatistics(apiParams);
    ...
    const revenueChange = useMemo(() => {
        if (!salesStats?.totalRevenue || !prevSalesStats?.totalRevenue)
            return 0;
        return (
            ((salesStats.totalRevenue - prevSalesStats.totalRevenue) /
    ...
    const ordersChange = useMemo(() => {
        if (!orderMetrics?.totalOrders || !prevOrderMetrics?.totalOrders)
            return 0;
        return (
            ((orderMetrics.totalOrders - prevOrderMetrics.totalOrders) /
                prevOrderMetrics.totalOrders) *
            100
        );
    ...
    const avgOrderValueChange = useMemo(() => {
        if (
            ...
            !prevOrderMetrics?.averageOrderValue
        )
            return 0;
        return (
            ((orderMetrics.averageOrderValue -
    ...
    const getActiveDateRange = () => {
        ...
        if (startDate === yesterdayStr && endDate === yesterdayStr)
            return 'yesterday';
        if (startDate === last7DaysStart && endDate === todayStr)
            return 'last7days';
        if (startDate === last30DaysStart && endDate === todayStr)
            return 'last30days';
    ...
    return (
        <div className="space-y-4 lg:space-y-6">
            <PageTitle icon={BarChart3} title="Sales Dashboard" />
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-2">
                <MetricCard
                    ...
                    value={
                        isLoading ? '...' : (productStats?.totalItemsSold ?? 0)
                    }
                    icon={Package}
                    isLoading={isLoading}
                ...
                <MetricCard
                    ...
                    value={
                        isLoading
                            ? '...'
                            : (productStats?.uniqueProductCount ?? 0)
                    }
                    icon={BarChart3}
                    isLoading={isLoading}
            ...
            <div className="space-y-6">
                ...
                <Card className="hover:shadow-lg transition-shadow duration-200">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Revenue and deposits over time (
                            {timePeriod.toLowerCase()}) with dual axis
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="p-6">
                        <div className="w-full">
                            <RevenueAreaChart
                                data={
                                    salesStats?.timeSeriesData?.map((item) => ({
                                        period: item.period,
                ...
                <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
                    ...
                    <Card className="hover:shadow-lg transition-shadow duration-200">
                        ...
                        <CardContent className="p-6">
                            <div className="w-full">
                                <StackedOrderChart
                                    data={
                                        orderMetrics?.timeSeriesData?.map(
                                            (item) => ({
                                                period: item.period,
                    ...
                    <Card className="hover:shadow-lg transition-shadow duration-200">
                        <CardHeader>
                            ...
                            <CardDescription>
                                Business hours activity heatmap (6h-23h) and
                                hourly distribution
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="p-6">
            ...
            {productStats?.timeSeriesData &&
                productStats.timeSeriesData.length > 0 && (
                    <Card className="hover:shadow-lg transition-shadow duration-200">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                            ...
                            <CardDescription>
                                Dual-axis view: quantity (bars) vs revenue
                                (line) with unique products indicator
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="p-6">
                            <div className="w-full">
                                <ProductDualAxisChart
                                    data={productStats.timeSeriesData.map(
                                        (item) => ({
                                            period: item.period,
            ...
            <div className="grid gap-6 grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
                ...
                {orderMetrics?.orderTypeMetrics &&
                    orderMetrics.orderTypeMetrics.length > 0 && (
                        <Card className="hover:shadow-lg transition-shadow duration-200">
                            <CardHeader>
                                <CardTitle className="flex items-center gap-2">
                                ...
                                <CardDescription>
                                    Breakdown by order type with detailed
                                    metrics
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                {orderMetrics.orderTypeMetrics.map(
                                    (orderType, index) => {
                                        const colors = [
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`p-4 rounded-lg border ${bgColors[index]} hover:shadow-md transition-shadow`}
                                            >
                                                <div className="flex justify-between items-start mb-3">
                                                    <div className="flex flex-col">
                                                        <span
                                                            className={`text-sm font-semibold ${textColors[index]}`}
                                                        >
                                                            {labels[
                                                                orderType.orderType as keyof typeof labels
                                                            ] ||
                                                                orderType.orderType}
                                                        </span>
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <div className="text-xs text-gray-600">
                                                            {orderType.revenuePercentage.toFixed(
                                                                1
                                                            )}
                                                            % revenue
                                                        </div>
                                                ...
                                                <div className="space-y-2">
                                                    <div className="flex justify-between text-xs font-medium text-gray-700">
                                                        <span>
                                                            Orders:{' '}
                                                            {orderType.orderPercentage.toFixed(
                                                                1
                                                            )}
                                                            %
                                                        </span>
                                                    ...
                                                    <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                                                        <div
                                                            ...
                                                            style={{
                                                                width: `${orderType.orderPercentage}%`,
                                                            }}
                                                        ></div>
                                                    </div>
                                                </div>
                ...
                {salesStats?.paymentTypeStats &&
                    salesStats.paymentTypeStats.length > 0 && (
                        <Card className="hover:shadow-lg transition-shadow duration-200">
                            <CardHeader>
                                <CardTitle className="flex items-center gap-2">
                                ...
                                <CardDescription>
                                    Payment distribution across different
                                    methods
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                {salesStats.paymentTypeStats.map(
                                    (payment, index) => {
                                        const colors = [
                                        ...
                                        return (
                                            <div
                                                ...
                                                className="space-y-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors"
                                            >
                                    ...
                                    .map((product, index) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className="p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg border hover:shadow-md transition-all duration-200"
                                            >
                                                <div className="flex items-center justify-between mb-3">
                                                    <div className="flex items-center gap-3">
                                                        <span
                                                            className={`text-sm font-bold px-3 py-1 rounded-full shadow-sm ${rankColors[index]}`}
                                                        >
                                                            #{index + 1}
                                                        </span>
                                                        <div className="flex flex-col">
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <div className="text-xs text-emerald-600 font-semibold">
                                                            {product.revenuePercentage.toFixed(
                                                                1
                                                            )}
                                                            % of total
                                                        </div>
                        ...
                        <CardContent className="text-center py-16">
                            <div className="max-w-md mx-auto">
                                ...
                                <p className="text-gray-600 mb-6 leading-relaxed">
                                    ...
                                    and filters. Try adjusting your filters or
