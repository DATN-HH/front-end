...
export default function FeedbackReportsPage() {
    ...
    const filteredData = feedbackData.filter(feedback => {
        ...
        const rangeStart = startOfDay(subDays(new Date(), parseInt(reportConfig.dateRange)));
        const rangeEnd = endOfDay(new Date());
        
        const isInDateRange = feedbackDate >= rangeStart && feedbackDate <= rangeEnd;
        const isInBranch = reportConfig.branch === 'ALL' || feedback.branchId.toString() === reportConfig.branch;
        const isInType = reportConfig.type === 'ALL' || feedback.feedbackType === reportConfig.type;
    ...
    const reportData = {
        summary: {
            ...
            positiveCount: filteredData.filter(f => f.overallRating >= 4).length,
            negativeCount: filteredData.filter(f => f.overallRating <= 2).length,
        },
        trends: {
            thisPeriod: filteredData.length,
            lastPeriod: Math.floor(filteredData.length * 0.85), // Mock comparison
        },
        byBranch: branchesData?.map(branch => ({
            name: branch.name,
            count: filteredData.filter(f => f.branchId === branch.id).length,
            avgRating: (() => {
                const branchFeedback = filteredData.filter(f => f.branchId === branch.id);
                return branchFeedback.length > 0 ? 
                    branchFeedback.reduce((sum, f) => sum + f.overallRating, 0) / branchFeedback.length : 0;
            })(),
        })) || [],
        topIssues: filteredData
            .filter(f => f.overallRating <= 2)
            .slice(0, 10),
        topPerformers: branchesData?.map(branch => {
            const branchFeedback = filteredData.filter(f => f.branchId === branch.id);
            const avgRating = branchFeedback.length > 0 ? 
                branchFeedback.reduce((sum, f) => sum + f.overallRating, 0) / branchFeedback.length : 0;
            return {
                name: branch.name,
                avgRating,
    ...
    const trendChange = reportData.trends.thisPeriod - reportData.trends.lastPeriod;
    const trendPercentage = reportData.trends.lastPeriod > 0 ? 
        ((trendChange / reportData.trends.lastPeriod) * 100) : 0;
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            if (reportConfig.format === 'EXCEL') {
                ...
                toast.error('Info', 'PDF export will be implemented with backend API');
            } else {
                generateJSONReport();
            }
        } catch (error) {
    ...
    const generateExcelReport = () => {
        ...
        const summaryData = [
            ...
            ['Branch', reportConfig.branch === 'ALL' ? 'All Branches' : 
                branchesData?.find(b => b.id.toString() === reportConfig.branch)?.name || 'Unknown'],
            ['Feedback Type', reportConfig.type === 'ALL' ? 'All Types' : reportConfig.type],
            [''],
        ...
        const feedbackHeaders = [
            'ID', 'Customer Name', 'Email', 'Phone', 'Branch', 'Product', 
            'Feedback Type', 'Overall Rating', 'Priority', 'Status', 
            'Title', 'Review Text', 'Response Text', 'Created At'
        ];
        ...
        const branchHeaders = ['Branch Name', 'Total Feedback', 'Average Rating', 'Response Rate'];
        ...
        const priorityHeaders = ['ID', 'Customer', 'Priority', 'Rating', 'Title', 'Created At', 'Status'];
        ...
        XLSX.utils.book_append_sheet(workbook, prioritySheet, 'Priority Issues');
    ...
    const generateCSVReport = () => {
        const headers = [
            'ID', 'Customer Name', 'Email', 'Branch', 'Product', 'Type', 
            'Rating', 'Priority', 'Status', 'Title', 'Review Text', 'Response Text', 'Created At'
        ];
        ...
        link.setAttribute('download', `feedback-report-${format(new Date(), 'yyyy-MM-dd')}.csv`);
        link.click();
    ...
    const generateJSONReport = () => {
        const reportContent = {
            metadata: {
                ...
                branch: reportConfig.branch === 'ALL' ? 'All Branches' : 
                    branchesData?.find(b => b.id.toString() === reportConfig.branch)?.name,
                type: reportConfig.type === 'ALL' ? 'All Types' : reportConfig.type,
        ...
        <div className="space-y-6">
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Select 
                                ...
                                onValueChange={(value) => setReportConfig({...reportConfig, branch: value})}
                            >
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{reportData.summary.totalFeedback}</div>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{reportData.summary.averageRating.toFixed(1)}</div>
                        <div className="flex mt-1">
                            {[1, 2, 3, 4, 5].map((star) => (
                                <Star
                                    key={star}
                                    size={14}
                                    className={star <= Math.round(reportData.summary.averageRating) ? 
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{reportData.summary.responseRate.toFixed(1)}%</div>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-green-600">{reportData.summary.positiveCount}</div>
                        <p className="text-xs text-muted-foreground">
                                            ...
                                            <div className="flex">
                                                {[1, 2, 3, 4, 5].map((star) => (
                                                    <Star
                                                        key={star}
                                                        size={12}
                                                        className={star <= Math.round(branch.avgRating) ? 
                                                            'fill-yellow-400 text-yellow-400' : 'text-gray-300'}
                                                    />
                                                ))}
                                            </div>
                                            <span>{branch.avgRating.toFixed(1)} avg</span>
                                        </div>
                                    </div>
                                    <Badge variant="outline">{branch.count} reviews</Badge>
                                </div>
                            ))}
                        </div>
                    </CardContent>
                        ...
                        <div className="space-y-4">
                            {reportData.topIssues.slice(0, 5).map((issue) => (
                                <div key={issue.id} className="flex items-start gap-3 p-3 border rounded-lg">
                                    <div className="flex">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <Star
                                                key={star}
                                                size={12}
                                                className={star <= issue.overallRating ? 
                                                    'fill-yellow-400 text-yellow-400' : 'text-gray-300'}
                                            />
                                        ))}
                                    </div>
                                    <div className="flex-1 space-y-1">
                                        ...
                                        <p className="text-xs text-gray-600 line-clamp-2">{issue.reviewText}</p>
                                    ...
                                    <Badge 
                                        variant={issue.feedbackStatus === 'PENDING' ? 'destructive' : 'secondary'}
                                        className="text-xs"
                                    >
                                        {issue.feedbackStatus}
                                    </Badge>
                            ...
                            {reportData.topIssues.length === 0 && (
                                <div className="text-center py-8 text-gray-500">
                                    ...
                                    <p className="text-sm">Great job maintaining quality service.</p>
                                </div>
                            )}
                        </div>
                    </CardContent>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        <Button variant="outline" className="flex items-center gap-2 h-auto p-4 flex-col">
                            ...
                            <div className="text-center">
                                ...
                                <div className="text-xs text-gray-500">Last 30 days summary</div>
                            </div>
                        </Button>
                        ...
                        <Button variant="outline" className="flex items-center gap-2 h-auto p-4 flex-col">
                            ...
                            <div className="text-center">
                                <div className="font-medium">Branch Comparison</div>
                                <div className="text-xs text-gray-500">Cross-branch analysis</div>
                            </div>
                        </Button>
                        ...
                        <Button variant="outline" className="flex items-center gap-2 h-auto p-4 flex-col">
                            ...
                            <div className="text-center">
                                ...
                                <div className="text-xs text-gray-500">Behavioral patterns</div>
                            </div>
                        </Button>
                    </div>
                </CardContent>
