...
export default function FoodCombosPage() {
    ...
    const filteredCombos = useMemo(() => {
        return combos.filter(combo => {
            // Keyword search
            if (keyword && !combo.name.toLowerCase().includes(keyword.toLowerCase()) &&
                !combo.description?.toLowerCase().includes(keyword.toLowerCase()) &&
                !combo.internalReference?.toLowerCase().includes(keyword.toLowerCase())) {
                return false;
            }
    ...
    const columns: ColumnDef<FoodComboResponse>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="space-y-1">
                    <div className="font-medium">{formatCurrency(row.original.effectivePrice)}</div>
                    {row.original.price && row.original.price !== row.original.effectivePrice && (
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="space-y-1">
                    <div className="font-medium">{formatCurrency(row.original.effectiveCost)}</div>
                    {row.original.cost && row.original.cost !== row.original.effectiveCost && (
                        <div className="text-xs text-gray-500">
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex gap-2">
                        <Link href={`/app/menu/food-combos/${row.original.id}/detail`}>
                ...
                <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <CardContent className="pt-0 flex-1 flex flex-col min-h-0">
                        ...
                        <div className="flex-1 space-y-3 min-h-0 overflow-y-auto">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center space-x-1">
                                    ...
                                    <span className="text-sm font-medium">{combo.itemsCount}</span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-sm font-medium">{formatCurrency(combo.effectivePrice)}</span>
                                </div>
                                {hasCustomPricing && (
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs text-gray-400">Override:</span>
                                        <span className="text-xs text-gray-400">{formatCurrency(combo.price)}</span>
                                    </div>
                                )}
                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-gray-500">Cost:</span>
                                    <span className="text-sm font-medium">{formatCurrency(combo.effectiveCost)}</span>
                                </div>
                            </div>

                            {/* Sales Info */}
                            <div className="flex items-center justify-between">
                                <Badge variant={combo.canBeSold ? 'default' : 'secondary'} className="text-xs">
                                    {combo.canBeSold ? 'Can Sell' : 'No Sell'}
                                </Badge>
                                <Badge variant={combo.availableInPos ? 'default' : 'secondary'} className="text-xs">
                        ...
                        <div className="space-y-2 mt-3 flex-shrink-0">
                            <div className="flex gap-2">
                                <Link href={`/app/menu/food-combos/${combo.id}/detail`} className="flex-1">
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-lg border space-y-4">
                <div className="flex items-center space-x-4">
                    ...
                    <select
                        value={canBeSold === undefined ? '' : canBeSold.toString()}
                        onChange={(e) => {
                            const value = e.target.value;
                            setCanBeSold(value === '' ? undefined : value === 'true');
                        }}
                        className="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                    ...
                    {(keyword || selectedStatus || canBeSold !== undefined || showArchived) && (
                        <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => {
                ...
                <div className="space-y-4">
                    ...
                    {isLoading ? (
                        ...
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-lg font-medium text-gray-900 mb-2">No food combos found</h3>
