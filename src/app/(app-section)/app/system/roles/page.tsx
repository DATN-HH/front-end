...
export function JobRoles() {
    ...
    const [currentRole, setCurrentRole] = useState<RoleResponseDto | null>(
        null
    );
    ...
    const { data: roleList, isLoading, error } = useRoles(queryParams);
    ...
    const handleEditRole = () => {
        if (currentRole) {
            const updateData: RoleUpdateDto = {
                ...
                rolePermissions: (currentRole.rolePermissions || []).map(
                    (p) => ({
                        id: p.id,
                        roleId: p.roleId,
