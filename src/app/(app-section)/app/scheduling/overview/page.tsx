...
export default function ScheduleOverviewPage() {
    ...
    const renderDateFilters = () => {
        return (
            <div className="space-y-4">
                ...
                {activeTab === 'weekly' && (
                    <div className="flex items-center gap-4">
                        <Label htmlFor="week">Week Start:</Label>
                        <Input
                            id="week"
                            type="date"
                            value={selectedWeek}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <Card>
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-2xl font-bold">{data.staffShiftStats.totalStaffShifts || 0}</p>
                ...
                <Card>
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-2xl font-bold">{data.staffWorkingStats.totalActiveStaff || 0}</p>
                ...
                <Card>
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-2xl font-bold">{data.leaveRequestStats.pendingRequests || 0}</p>
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-blue-50 p-3 rounded-lg text-center">
                                    <div className="text-2xl font-bold text-blue-600">{data.staffShiftStats.publishedShifts || 0}</div>
                                    <div className="text-xs text-blue-600 font-medium">Published</div>
                                </div>
                                <div className="bg-yellow-50 p-3 rounded-lg text-center">
                                    <div className="text-2xl font-bold text-yellow-600">{data.staffShiftStats.pendingShifts || 0}</div>
                                    <div className="text-xs text-yellow-600 font-medium">Pending</div>
                                </div>
                                <div className="bg-gray-50 p-3 rounded-lg text-center">
                                    <div className="text-2xl font-bold text-gray-600">{data.staffShiftStats.draftShifts || 0}</div>
                                    <div className="text-xs text-gray-600 font-medium">Draft</div>
                                </div>
                                <div className="bg-red-50 p-3 rounded-lg text-center">
                                    <div className="text-2xl font-bold text-red-600">{data.staffShiftStats.conflictedShifts || 0}</div>
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-green-50 p-3 rounded-lg text-center">
                                    <div className="text-2xl font-bold text-green-600">{data.leaveRequestStats.approvedRequests || 0}</div>
                                    <div className="text-xs text-green-600 font-medium">Approved</div>
                                </div>
                                <div className="bg-yellow-50 p-3 rounded-lg text-center">
                                    <div className="text-2xl font-bold text-yellow-600">{data.leaveRequestStats.pendingRequests || 0}</div>
                                    <div className="text-xs text-yellow-600 font-medium">Pending</div>
                                </div>
                                <div className="bg-red-50 p-3 rounded-lg text-center">
                                    <div className="text-2xl font-bold text-red-600">{data.leaveRequestStats.rejectedRequests || 0}</div>
                                    <div className="text-xs text-red-600 font-medium">Rejected</div>
                                </div>
                                <div className="bg-gray-50 p-3 rounded-lg text-center">
                                    <div className="text-2xl font-bold text-gray-600">{data.leaveRequestStats.cancelledRequests || 0}</div>
                    ...
                    <CardContent>
                        <div className="space-y-3 max-h-64 overflow-y-auto">
                            {data.staffWorkingStats.workingStaff.slice(0, 10).map((staff) => (
                                <div key={staff.staffId} className="flex justify-between items-center p-2 border rounded">
                                    <div>
                            ...
                            <div className="space-y-3 max-h-64 overflow-y-auto">
                                {data.staffWorkingStats.nonWorkingStaff.slice(0, 10).map((staff) => (
                                    <div key={staff.staffId} className="flex justify-between items-center p-2 border rounded">
                                        ...
                                        <Badge variant="outline">No shifts</Badge>
                                    </div>
                                ))}
    ...
    return (
        <div className="space-y-6">
            ...
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as ViewType)} className="space-y-6">
