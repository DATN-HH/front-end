...
export function FloorCanvas({
    ...
    const [pendingUpdates, setPendingUpdates] = useState<Record<number, { x?: number; y?: number; width?: number; height?: number }>>({});
    const processedUpdatesRef = useRef<Record<number, { x?: number; y?: number; width?: number; height?: number }>>({});
    ...
    const calculateImageSize = (containerWidth: number, containerHeight: number, imageWidth: number, imageHeight: number) => {
        ...
        const updateSize = () => {
            if (canvasRef.current) {
                ...
                if (floor.imageUrl) {
                    const img = new Image();
                    img.onload = () => {
                        const calculatedSize = calculateImageSize(rect.width, rect.height, img.naturalWidth, img.naturalHeight);
    ...
    useEffect(() => {
        ...
        const hasChanges = Object.entries(debouncedUpdates).some(([tableId, update]) => {
            const processed = processedUpdatesRef.current[parseInt(tableId)];
            return !processed ||
        ...
        Object.entries(debouncedUpdates).forEach(([tableId, update]) => {
            if (update.x !== undefined && update.y !== undefined) {
                onTableDrop(parseInt(tableId), { x: update.x, y: update.y }, imageSize);
            }
            if (update.width !== undefined && update.height !== undefined) {
                onTableResize(parseInt(tableId), { width: update.width, height: update.height }, imageSize);
            }
        });
        ...
        // Clear pending updates after a delay to prevent the infinite loop
        setTimeout(() => {
            setPendingUpdates(prev => {
                const newPending = { ...prev };
                Object.keys(debouncedUpdates).forEach(tableId => {
                    delete newPending[parseInt(tableId)];
                });
                return newPending;
            });
        }, 100);
    ...
    const ratioToPixelsPosition = (ratio: number, dimension: 'width' | 'height') => {
                    ...
                    return (
                        <Rnd
                            ...
                            size={{
                                ...
                                height: pendingUpdates[table.id]?.height ?? height
                            }}
                            position={{
                            ...
                            onDragStop={isBookingMode ? () => { } : (e, d) => {
                                ...
                                const newPosition = {
                                    ...
                                    y: Math.max(imageSize.offsetY, Math.min(imageSize.offsetY + imageSize.height - height, d.y))
                                };
                                setPendingUpdates(prev => ({
                                    ...prev,
                            ...
                            onResizeStop={isBookingMode ? () => { } : (e, direction, ref, delta, position) => {
                                ...
                                setPendingUpdates(prev => ({
                                    ...prev,
                                    [table.id]: { ...prev[table.id], width: newSize.width, height: newSize.height }
                                }));
                            }}
                            bounds="parent"
