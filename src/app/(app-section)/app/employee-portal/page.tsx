  ...
  return (
    <div className="space-y-4 lg:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            ...
            >
              ...
              <span className="hidden sm:inline">
                Pending Shifts{' '}
                {pendingShifts.length > 0 && `(${pendingShifts.length})`}
              </span>
      ...
      <p className="text-muted-foreground -mt-2 lg:-mt-4 text-sm lg:text-base">
        Welcome back, {user?.fullName}! Here's your work overview and quick
        actions.
      </p>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex items-center gap-2 text-red-800 flex-1">
                  ...
                  <span className="font-medium text-sm">
                    {urgentPendingShifts.length} urgent shift response
                    {urgentPendingShifts.length !== 1 ? 's' : ''} needed
                    (deadline &lt; 24h)
                  </span>
                </div>
                <Button
          ...
          <Card className="border-l-4 border-l-red-500 bg-red-50">
            <CardContent className="p-4">
              <div className="flex items-center gap-2 text-red-800">
                <AlertTriangle className="h-4 w-4 flex-shrink-0" />
                <span className="font-medium text-sm">
                  Low shift leave balance: {shiftLeaveBalance.availableShifts}{' '}
                  shift
                  {shiftLeaveBalance.availableShifts !== 1 ? 's' : ''} remaining
                </span>
              </div>
            </CardContent>
          </Card>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-lg lg:text-2xl font-bold">
                  {isHours7Loading ? '...' : workingHours7?.totalHours || 0}h
                </p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-lg lg:text-2xl font-bold">
                  {isHours30Loading ? '...' : workingHours30?.totalHours || 0}h
                </p>
                <p className="text-xs lg:text-sm text-muted-foreground">
                  Last 30 Days
                </p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-col sm:flex-row sm:justify-between text-sm gap-1">
                      ...
                      <span>
                        Remaining: {shiftLeaveBalance.availableShifts} shifts
                      </span>
                    </div>
                    <Progress
            ...
            <CardContent className="p-4 lg:p-6 pt-0">
              {isShiftLeaveRequestsLoading ? (
                ...
                <div className="text-center text-muted-foreground py-8">
                  No shift leave requests yet. Click "Shift Leave" to get
                  started.
                </div>
              ) : (
                <div className="space-y-4">
            ...
            <CardContent className="space-y-4 p-4 lg:p-6 pt-0">
              ...
              {shiftLeaveBalance && shiftLeaveBalance.availableShifts <= 5 && (
                <div className="flex items-center gap-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  ...
                  <p className="text-sm text-yellow-800">
                    Low shift leave balance: {shiftLeaveBalance.availableShifts}{' '}
                    shift
                    {shiftLeaveBalance.availableShifts !== 1 ? 's' : ''}{' '}
                    remaining
                  </p>
                </div>
              )}
              ...
              {pendingShiftLeaveRequests.length > 0 && (
                <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <p className="text-sm text-blue-800">
                    You have {pendingShiftLeaveRequests.length} pending shift
                    leave request
                    {pendingShiftLeaveRequests.length !== 1 ? 's' : ''}
                  </p>
                </div>
              )}
              ...
              {pendingShifts.length > 0 && (
                <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <Clock className="h-4 w-4 text-blue-600 flex-shrink-0" />
                  <p className="text-sm text-blue-800">
                    You have {pendingShifts.length} pending shift
