...
export default function EmployeeShiftLeavePage() {
    ...
    const { data: myRequests, isLoading: isLoadingRequests } = useMyShiftLeaveRequests(currentYear);
    const { data: myBalance, isLoading: isLoadingBalance } = useMyShiftLeaveBalance(currentYear);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.startDate || !formData.endDate || !formData.reason || formData.selectedShiftIds.length === 0) {
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: ['my-shift-leave-requests'] });
            queryClient.invalidateQueries({ queryKey: ['my-shift-leave-balance'] });
        } catch (err: any) {
            error('Error', err?.response?.data?.message || 'Failed to create request');
        }
    };
    ...
    const handleCancelRequest = async (requestId: number) => {
        try {
            ...
            error('Error', err?.response?.data?.message || 'Failed to cancel request');
        }
    };
    ...
    return (
        <div className="space-y-4 lg:space-y-6">
                    ...
                    {isLoadingBalance ? (
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
                            <div className="text-center p-3 lg:p-4">
                                <div className="text-xl lg:text-2xl font-bold text-blue-600">{balance.totalShifts}</div>
                                <div className="text-xs lg:text-sm text-gray-600">Total Shifts</div>
                            </div>
                            <div className="text-center p-3 lg:p-4">
                                <div className="text-xl lg:text-2xl font-bold text-red-600">{balance.usedShifts}</div>
                                <div className="text-xs lg:text-sm text-gray-600">Used</div>
                            </div>
                            <div className="text-center p-3 lg:p-4">
                                <div className="text-xl lg:text-2xl font-bold text-green-600">{balance.bonusShifts}</div>
                                <div className="text-xs lg:text-sm text-gray-600">Bonus</div>
                            </div>
                            <div className="text-center p-3 lg:p-4">
                                <div className="text-xl lg:text-2xl font-bold text-purple-600">{balance.availableShifts}</div>
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="text-base lg:text-lg">Shift Leave Requests</CardTitle>
                </CardHeader>
                <CardContent>
                    {isLoadingRequests ? (
                        <div>Loading...</div>
                                ...
                                <div key={request.id} className="border rounded-lg p-3 lg:p-4">
                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-3">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs lg:text-sm text-gray-600 mt-1 break-words">{request.reason}</p>
                                        </div>
                                        <Badge className={`${getStatusColor(request.requestStatus)} whitespace-nowrap`}>
                                    ...
                                    {request.affectedShiftsCount > 0 && (
                                        <div className="flex items-center gap-2 text-xs lg:text-sm text-green-600 mb-2">
                                            ...
                                            <span>Affected shifts: {request.affectedShiftsCount}</span>
                                        </div>
                                    )}
                                    ...
                                    {request.managerNote && (
                                        <div className="flex items-start gap-2 text-xs lg:text-sm text-blue-600 mb-2">
                                            ...
                                            <span className="break-words">Manager note: {request.managerNote}</span>
                                        </div>
                                    )}

                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 pt-2 border-t">
                                        <p className="text-xs text-gray-500">
                                            Created: {format(new Date(request.createdAt), 'dd/MM/yyyy HH:mm')}
                                        </p>
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-sm">{format(new Date(selectedRequest.startDate), 'dd/MM/yyyy')} - {format(new Date(selectedRequest.endDate), 'dd/MM/yyyy')}</p>
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <p className="text-sm mt-1 break-words">{selectedRequest.reason}</p>
                            </div>
                            ...
                            {selectedRequest.affectedShiftsCount > 0 && (
                                <div>
                                    ...
                                    <p className="text-sm mt-1">{selectedRequest.affectedShiftsCount} shifts</p>
                                </div>
                            )}
                            ...
                            {selectedRequest.managerNote && (
                                <div>
                                    ...
                                    <p className="text-sm mt-1 break-words">{selectedRequest.managerNote}</p>
                                </div>
                            )}
                            ...
                            {selectedRequest.approvedBy && (
                                <div>
                                    ...
                                    <p className="text-sm mt-1">{selectedRequest.approvedBy.fullName}</p>
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-xs lg:text-sm text-gray-600 pt-2 border-t">
                                ...
                                {selectedRequest.isManagerAdded && (
                                    <Badge variant="outline" className="text-xs">Manager Added</Badge>
                                )}
                            </div>
                        </div>
                    )}
