            ...
            <DialogContent className="max-w-md">
                ...
                {selectedEmployee && (
                    <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                        <div className="font-medium">{selectedEmployee.user.fullName}</div>
                        <div className="text-sm text-gray-600">{selectedEmployee.user.email}</div>
                        <div className="text-xs text-gray-500 mt-1">
                            Current Available: {selectedEmployee.availableShifts} shifts
                        </div>
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <p className="text-lg font-semibold text-blue-600">{currentYear}</p>
                        <p className="text-sm text-gray-500">Balance will be updated for current year</p>
                    </div>

                    <div>
                        <Label htmlFor="bonusShifts">Bonus Shifts (can be negative)</Label>
                        <Input
                            id="bonusShifts"
                            type="number"
                            value={formData.bonusShifts}
                            onChange={(e) => setFormData(prev => ({ ...prev, bonusShifts: parseInt(e.target.value) || 0 }))}
                            required
