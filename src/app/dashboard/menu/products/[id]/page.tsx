...
const product = {
  ...
  variants: [
    { id: 1, name: "Phở Bò Tái - Nhỏ", price: 45000, attributes: "Kích cỡ: Nhỏ" },
    { id: 2, name: "Phở Bò Tái - Vừa", price: 50000, attributes: "Kích cỡ: Vừa" },
    { id: 3, name: "Phở Bò Tái - Lớn", price: 60000, attributes: "Kích cỡ: Lớn" },
  ],
}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Chi tiết sản phẩm và cấu hình</p>
          </div>
        </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{formatCurrency(product.cost)}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium text-gray-500">Mã nội bộ</label>
                    <p className="text-sm font-medium">{product.internalReference}</p>
                  </div>
                  <div>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <Badge variant={product.active ? "default" : "secondary"}>{product.active ? "Có" : "Không"}</Badge>
                </div>
                <div>
                ...
                <div>
                  <label className="text-sm font-medium text-gray-500">Có thể mua</label>
                  <Badge variant={product.canBePurchased ? "default" : "secondary"}>
                    {product.canBePurchased ? "Có" : "Không"}
                  </Badge>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Có sẵn trong POS</label>
                  <Badge variant={product.availableInPos ? "default" : "secondary"}>
                    {product.availableInPos ? "Có" : "Không"}
                  </Badge>
                </div>
              </div>
            </CardContent>
        ...
        <TabsContent value="variants" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Danh sách các biến thể của sản phẩm này</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {product.variants.map((variant) => (
                  <div key={variant.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{variant.attributes}</p>
                    </div>
        ...
        <TabsContent value="sales" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-medium text-gray-500">Mô tả bán hàng</label>
                  <p className="text-sm">{product.description}</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium text-gray-500">Chính sách hóa đơn</label>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-500">Danh mục POS</label>
                    <p className="text-sm font-medium">{product.posCategory}</p>
                  </div>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-500">Theo dõi tồn kho</label>
                    <Badge variant="secondary">{product.type === "Stockable" ? "Có" : "Không"}</Badge>
                  </div>
                </div>
              </div>
            </CardContent>
