...
export function PendingShiftsModal({
  open,
  onOpenChange,
}: PendingShiftsModalProps) {
  const { success, error } = useCustomToast();
  const [selectedShift, setSelectedShift] = useState<StaffShiftFeedback | null>(
    null
  );
  ...
  const handleApprove = async (shift: StaffShiftFeedback) => {
    try {
      ...
      success(
        'Shift Approved',
        `You have accepted the shift on ${format(parseISO(shift.staffShift.scheduledShift.date), 'dd/MM/yyyy')}`
      );
      refetch();
    } catch (err: any) {
      error(
        'Response Failed',
        err.response?.data?.message || 'Failed to respond to shift'
      );
    }
  };
  ...
  const handleReject = async () => {
    ...
    if (!rejectionReason.trim()) {
      error(
        'Reason Required',
        'Please provide a reason for rejecting this shift'
      );
      return;
    }
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 pb-4 border-b">
                  ...
                  <h2 className="text-xl font-semibold text-foreground">
                    ...
                    {pendingShifts.length !== 1 ? 's' : ''} awaiting response
                  </h2>
                </div>
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              ...
                              <CardDescription className="mt-2 text-muted-foreground">
                                {format(
                                  parseISO(
                                    shift.staffShift.scheduledShift.date
                                  ),
                                  'EEEE, MMMM dd, yyyy'
                                )}{' '}
                                • {shift.staffShift.scheduledShift.startTime} -{' '}
                                {shift.staffShift.scheduledShift.endTime}
                              </CardDescription>
                            </div>
                        ...
                        <CardContent className="space-y-4 rounded-lg">
                          ...
                          <div className="flex gap-3 pt-4 border-t">
                            <Button
                              ...
                              disabled={
                                respondMutation.isPending || deadline.isExpired
                              }
      ...
      <AlertDialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Please provide a reason for rejecting this shift. This will be
            ...
            {selectedShift && (
              <div className="p-3 bg-muted rounded-lg">
                ...
                <div className="text-sm text-muted-foreground">
                  {format(
                    parseISO(selectedShift.staffShift.scheduledShift.date),
                    'EEEE, MMMM dd, yyyy'
                  )}{' '}
                  • {selectedShift.staffShift.scheduledShift.startTime} -{' '}
                  {selectedShift.staffShift.scheduledShift.endTime}
                </div>
              </div>
            )}
