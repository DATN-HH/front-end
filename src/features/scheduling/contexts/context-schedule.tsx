...
export const ScheduleProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const [isCreateOpenShiftDialogOpen, setIsCreateOpenShiftDialogOpen] = useState(false);
    ...
    const { data: shifts, isLoading: isLoadingShifts } = useShifts(user?.branch?.id ?? 0);
    const { data: scheduledShifts, isLoading: isLoadingScheduledShifts } = useScheduledShifts({
        branchId: user?.branch?.id ?? 0,
        startDate: dayjs(startDate).format("YYYY-MM-DD"),
        endDate: dayjs(endDate).format("YYYY-MM-DD"),
    });
    const { data: scheduledShiftsGrouped, isLoading: isLoadingScheduledShiftsGrouped } = useScheduledShiftsGrouped({
        branchId: user?.branch?.id ?? 0,
        startDate: dayjs(startDate).format("YYYY-MM-DD"),
        endDate: dayjs(endDate).format("YYYY-MM-DD"),
    });
    ...
    const { data: staffShiftsGrouped, isLoading: isLoadingStaffShiftsGrouped } = useStaffShiftsGrouped({
        branchId: user?.branch?.id ?? 0,
        startDate: dayjs(startDate).format("YYYY-MM-DD"),
        endDate: dayjs(endDate).format("YYYY-MM-DD"),
        size: 1000000
