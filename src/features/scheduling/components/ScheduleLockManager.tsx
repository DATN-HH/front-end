...
export function ScheduleLockManager({ open, onOpenChange }: ScheduleLockManagerProps) {
    const { success, error } = useCustomToast();
    const { user } = useAuth();
    const branchId = user?.branch?.id;

    // State declarations first
    const [activeTab, setActiveTab] = useState<'lock' | 'manage' | 'check'>('lock');
    ...
    const { data: rangelocks, isLoading: isLoadingRange } = useScheduleLocksInRange(
        branchId!,
        rangeStartDate,
        rangeEndDate
    );

    const { data: activeLockForDate } = useActiveScheduleLock(branchId!, checkDate);
    ...
    const handleLockSubmit = async () => {
        ...
        try {
            ...
            error('Lock Failed', err.response?.data?.message || 'Failed to lock schedule');
        }
    };
    ...
    const handleUnlockSubmit = async () => {
        ...
        try {
            ...
            error('Unlock Failed', err.response?.data?.message || 'Failed to unlock schedule');
        }
    };
    ...
    const lockHistory = locks?.filter(lock => lock.lockStatus !== ScheduleLockStatus.LOCKED) || [];
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader className="border-b pb-6">
                    <DialogTitle className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-foreground">Schedule Lock Manager</h1>
                ...
                <div className="space-y-8 py-6">
                    ...
                    <div className="flex space-x-2 bg-muted p-2 rounded-xl">
                        <button
                            ...
                            className={`flex-1 px-4 py-3 text-sm font-medium rounded-lg transition-colors ${activeTab === 'lock'
                        ...
                        <button
                            ...
                            className={`flex-1 px-4 py-3 text-sm font-medium rounded-lg transition-colors ${activeTab === 'manage'
                                ? 'bg-card text-foreground shadow-sm'
                                : 'text-muted-foreground hover:text-foreground hover:bg-muted-foreground/10'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex-1 px-4 py-3 text-sm font-medium rounded-lg transition-colors ${activeTab === 'check'
                                ? 'bg-card text-foreground shadow-sm'
                                : 'text-muted-foreground hover:text-foreground hover:bg-muted-foreground/10'
                                }`}
                        >
                            ...
                            <Card className="border-2 border-primary/20 bg-primary/5">
                                <CardHeader className="pb-4">
                                    ...
                                    <CardDescription className="text-muted-foreground">
                                        Lock a schedule for a specific date range to prevent modifications
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-6 rounded-lg">
                                    ...
                                    <div className="space-y-3">
                                        <Label htmlFor="lockReason" className="text-sm font-medium text-foreground">Lock Reason (Optional)</Label>
                                        <Textarea
                                            id="lockReason"
                                            placeholder="Enter reason for locking schedule..."
                                            value={lockForm.lockReason}
                                            onChange={(e) => setLockForm(prev => ({ ...prev, lockReason: e.target.value }))}
                                        />
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="pt-6">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="text-sm text-destructive">
                                            <div className="font-semibold mb-2 text-foreground">Important Notice:</div>
                                            <ul className="space-y-2 text-sm">
                                                ...
                                                <li className="flex items-start gap-2">
                                                    <span className="text-destructive mt-1">•</span>
                                                    <span>Consider the date range carefully before locking</span>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                                                    ...
                                                                    <div className="space-y-2">
                                                                        <div className="flex items-center gap-2">
                                                                            ...
                                                                            <span className="font-medium">
                                                                                {format(new Date(lock.startDate), 'MMM dd, yyyy')} - {format(new Date(lock.endDate), 'MMM dd, yyyy')}
                                                                            </span>
                                                                        ...
                                                                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                                                            ...
                                                                            <span>{format(new Date(lock.lockedAt), 'MMM dd, yyyy HH:mm')}</span>
                                                                        </div>
                                                                        {lock.lockReason && (
                                                                            <div className="text-sm text-muted-foreground">
                                                                                <strong>Reason:</strong> {lock.lockReason}
                                                                            </div>
                                                                        )}
                                                                    </div>
                                                                    <Button
                                            ...
                                            <CardContent className="space-y-4">
                                                <div className="space-y-2">
                                                    ...
                                                    <Textarea
                                                        ...
                                                        onChange={(e) => setUnlockForm(prev => ({ ...prev, unlockReason: e.target.value }))}
                                                    />
                                                </div>
                                                ...
                                                <CardContent>
                                                    <div className="space-y-3">
                                                        {lockHistory.map((lock) => (
                                                            <div key={lock.id} className="flex items-center justify-between p-3 border rounded">
                                                                <div className="space-y-1">
                                                                    <div className="flex items-center gap-2">
                                                                        <Calendar className="h-4 w-4 text-muted-foreground" />
                                                                        <span className="text-sm font-medium">
                                                                            {format(new Date(lock.startDate), 'MMM dd')} - {format(new Date(lock.endDate), 'MMM dd, yyyy')}
                                                                        </span>
                                                                        {getStatusBadge(lock.lockStatus)}
                                                                    </div>
                                                                    <div className="text-xs text-muted-foreground">
                                                                        Locked by {lock.lockedByName} • Unlocked by {lock.unlockedByName}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        ))}
                        ...
                        <div className="space-y-6">
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        Check if a specific date is locked and view active lock details
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="space-y-2">
                                    ...
                                    {checkDate && (
                                        <div className="space-y-4 mt-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <Badge variant={isDateLocked ? "destructive" : "success"}>
                                            ...
                                            {activeLockForDate && (
                                                <Card className="border-orange-200 bg-orange-50">
                                                    <CardHeader>
                                                        <CardTitle className="text-lg text-orange-900">Active Lock Details</CardTitle>
                                                    </CardHeader>
                                                    <CardContent>
                                                        <div className="space-y-2">
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <span className="font-medium">
                                                                    {format(new Date(activeLockForDate.startDate), 'MMM dd, yyyy')} - {format(new Date(activeLockForDate.endDate), 'MMM dd, yyyy')}
                                                                </span>
                                                            </div>
                                                            <div className="flex items-center gap-2">
                                                                <User className="h-4 w-4" />
                                                                <span>Locked by: {activeLockForDate.lockedByName}</span>
                                                            </div>
                                                            <div className="flex items-center gap-2">
                                                                <Clock className="h-4 w-4" />
                                                                <span>Locked at: {format(new Date(activeLockForDate.lockedAt), 'MMM dd, yyyy HH:mm')}</span>
                                                            </div>
                                                            {activeLockForDate.lockReason && (
                                                                <div className="text-sm">
                                                                    <strong>Reason:</strong> {activeLockForDate.lockReason}
                                                                </div>
                                                            )}
                                                        </div>
                                                    </CardContent>
