...
import { Edit, Trash, Clock, Users, Calendar, MapPin, CheckCircle } from 'lucide-react';
            ...
            cell: ({ row }) => (
                <div className="flex flex-wrap items-center gap-1.5 min-w-[180px]">
                    ...
                    {row.original.requirements.length === 0 && (
                        <span className="text-xs text-muted-foreground italic">No requirements</span>
                    )}
                </div>
            ),
        },
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex items-center gap-2">
                        <div className={`p-1.5 rounded-full ${isActive ? 'bg-primary/10' : 'bg-muted'}`}>
                            <CheckCircle className={`h-3.5 w-3.5 ${isActive ? 'text-primary' : 'text-muted-foreground'}`} />
                        </div>
                        <Badge
            ...
            cell: ({ row }) => (
                <div className="flex flex-col text-sm min-w-[100px]">
                    ...
                    <span className="text-xs text-muted-foreground">
                        ...
                        <span className="ml-1 text-muted-foreground/70">({dayjs(row.original.updatedAt).fromNow()})</span>
                    </span>
                </div>
            ),
        },
