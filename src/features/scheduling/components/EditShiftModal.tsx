    ...
    useEffect(() => {
        if (isOpen && shift) {
            ...
            const parseTime = (time: any) => {
                if (typeof time === 'string') {
                    ...
                    return {
                        ...
                        minute: time.minute?.toString().padStart(2, '0') || '00'
                    };
                }
            ...
            if (shift.requirements) {
                setPositions(shift.requirements.map(req => ({
                    name: req.role,
            ...
            const newSelectedDays = daysOfWeek.reduce((acc, day) => ({ ...acc, [day.id]: false }), {} as Record<string, boolean>);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const updatedShift: ShiftResponseDto = {
                ...shift,
                name: shiftName,
                startTime: formatTime(startHour, startMinute) as unknown as LocalTime,
            ...
            customToast.error("Error", "Failed to update shift: " + (error as Error).message);
        } finally {
            setIsSubmitting(false);
        }
    };
