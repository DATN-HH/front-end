...
export function ShiftAssignmentSuggestions({ open, onOpenChange }: ShiftAssignmentSuggestionsProps) {
    ...
    const [schedule, setSchedule] = useState<CompleteScheduleResponse | null>(null);
    ...
    const handleGenerateSchedule = async () => {
        ...
        try {
            ...
            setSchedule(result);
            success('Schedule Analyzed', `Successfully analyzed schedule for week ${format(weekStart, 'MMM dd')} - ${format(weekEnd, 'MMM dd')}`);
        } catch (err: any) {
            error('Analysis Failed', err.response?.data?.message || 'Failed to analyze schedule');
        }
    };

    const handleCreateStaffShift = async (staff: StaffAssignment, scheduledShiftId: number, shiftName: string) => {
        try {
            ...
            success('Shift Assigned', `Successfully assigned ${staff.staffName} to ${shiftName}`);
        } catch (err: any) {
            error('Assignment Failed', err.response?.data?.message || 'Failed to assign staff to shift');
        }
    };

    const handleBulkAssignRole = async (roleAssignment: RoleAssignment, scheduledShiftId: number, shiftName: string) => {
        const suggestedStaff = roleAssignment.assignedStaff.filter(staff => !staff.isExisting);
        ...
        const assignments: BulkStaffShiftAssignment[] = suggestedStaff.map((staff, index) => ({
            staffId: staff.staffId,
            scheduledShiftId: scheduledShiftId,
            shiftStatus: 'DRAFT',
        ...
        try {
            ...
            error('Bulk Assignment Failed', err.response?.data?.message || 'Failed to assign suggested staff');
        }
    };
    ...
    const handleBulkAssignAllSuggestions = async () => {
        ...
        if (allAssignments.length === 0) {
            error('No Suggestions', 'No suggested staff assignments found for this week');
            return;
        }
        ...
        try {
            ...
            error('Bulk Assignment Failed', err.response?.data?.message || 'Failed to assign all suggested staff');
        }
    };
    ...
    const renderRoleAssignment = (roleAssignment: RoleAssignment, scheduledShiftId: number, shiftName: string) => {
        const existingStaff = roleAssignment.assignedStaff.filter(staff => staff.isExisting);
        const suggestedStaff = roleAssignment.assignedStaff.filter(staff => !staff.isExisting);

        return (
            <div key={roleAssignment.roleName} className="border rounded-md p-2 space-y-2">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 min-w-0 flex-1">
                        <Badge variant={roleAssignment.isFullyFilled ? "default" : "destructive"} className="text-xs">
                            {roleAssignment.roleName}
                        </Badge>
                        <div className="text-xs text-muted-foreground ml-1 flex items-center gap-1">
                            <span className="text-green-600 font-medium">{roleAssignment.existingQuantity}</span>
                            {roleAssignment.suggestedQuantity > 0 && (
                                <>
                                    <span>+</span>
                                    <span className="text-blue-600 font-medium">{roleAssignment.suggestedQuantity}</span>
                                </>
                            )}
                    ...
                    {!roleAssignment.isFullyFilled && (
                        <Badge variant="outline" className="text-orange-600 border-orange-600 text-xs flex-shrink-0">
                            Need {roleAssignment.requiredQuantity - roleAssignment.assignedQuantity}
                        </Badge>
                    )}
                </div>
                ...
                <div className="space-y-1">
                    ...
                    {existingStaff.length > 0 && (
                        <div className="space-y-1">
                            <div className="text-xs font-medium text-green-700 mb-1">✅ Already Assigned</div>
                            {existingStaff.map((staff) => (
                                <div key={staff.staffId} className="flex items-center justify-between p-2 bg-green-50 border border-green-200 rounded text-xs">
                                    <div className="flex items-center gap-2 flex-1 min-w-0">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="font-medium truncate text-green-800">{staff.staffName}</div>
                    ...
                    {suggestedStaff.length > 0 && (
                        <div className="space-y-1">
                            <div className="text-xs font-medium text-blue-700 mb-1">➕ Suggested to Add</div>
                            {suggestedStaff.map((staff) => (
                                <div key={staff.staffId} className="flex items-center justify-between p-2 bg-blue-50 border border-blue-200 rounded text-xs">
                                    <div className="flex items-center gap-2 flex-1 min-w-0">
                                        <Avatar className="h-6 w-6 flex-shrink-0">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="font-medium truncate text-blue-800">{staff.staffName}</div>
                                    ...
                                    <Button
                                        size="sm"
                                        onClick={() => handleCreateStaffShift(staff, scheduledShiftId, shiftName)}
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded p-2">
                        <div className="flex items-start gap-1">
                            ...
                            <div className="text-xs">
                                <div className="font-medium text-yellow-800">Suggestion Warnings:</div>
    ...
    const renderDaySchedule = (date: Date) => {
        ...
        return (
            <div className="space-y-2">
                {shifts.map((shift) => (
                    <Card key={shift.scheduledShiftId} className="border-l-2 border-l-primary">
                        ...
                        <CardContent className="space-y-2 px-3 pb-3">
                            {shift.roleAssignments.map((roleAssignment) =>
                                renderRoleAssignment(roleAssignment, shift.scheduledShiftId, shift.shiftName)
                            )}
            ...
            <DialogContent className="max-w-[95vw] max-h-[95vh] overflow-y-auto">
                <DialogHeader className="border-b pb-6">
                    <DialogTitle className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-foreground">Weekly Schedule Suggestions</h1>
                            <p className="text-base text-muted-foreground mt-1">
                                View existing assignments and get suggestions for missing staff positions
                            </p>
                        </div>
                    </DialogTitle>
                </DialogHeader>
                            ...
                            <div className="flex items-center justify-center gap-3">
                                ...
                                <div className="text-center min-w-[250px]">
                                    ...
                                    <div className="text-xs text-muted-foreground">
                                        Week {format(weekStart, 'w', { locale: enUS })} of {format(weekStart, 'yyyy')}
                                    </div>
                                </div>
                                <Button
                                    variant="outline"
                            ...
                            <CardContent>
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                                    <div className="text-center p-3 bg-white rounded border">
                                        <div className="text-xl font-bold text-primary">{schedule.summary.totalShifts}</div>
                                        <div className="text-xs text-muted-foreground">Total Shifts</div>
                                    </div>
                                    <div className="text-center p-3 bg-white rounded border">
                                        <div className="text-xl font-bold text-green-600">{schedule.summary.fullyStaffedShifts}</div>
                                        <div className="text-xs text-muted-foreground">Fully Staffed</div>
                                    </div>
                                    <div className="text-center p-3 bg-white rounded border">
                                        <div className="text-xl font-bold text-yellow-600">{schedule.summary.partiallyStaffedShifts}</div>
                                        <div className="text-xs text-muted-foreground">Partially Staffed</div>
                                    </div>
                                    <div className="text-center p-3 bg-white rounded border">
                                        <div className="text-xl font-bold text-blue-600">{schedule.summary.scheduleCompleteness.toFixed(1)}%</div>
                                ...
                                {schedule.warnings.length > 0 && (
                                    <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded p-3">
                                        <div className="flex items-start gap-2">
                                            ...
                                            <div>
                                                <div className="font-medium text-yellow-800 mb-1 text-sm">Warnings:</div>
                        ...
                        <Card className="border-2 border-dashed border-muted">
                            <CardContent className="pt-12 pb-12">
                                <div className="text-center text-muted-foreground">
                                    ...
                                    <h3 className="text-lg font-medium text-foreground mb-2">No schedule analysis yet</h3>
                                    <p className="text-sm text-muted-foreground">
                                        Select a week and click "Generate Schedule" to view assignments and suggestions
                                    </p>
                                </div>
                            </CardContent>
                        </Card>
