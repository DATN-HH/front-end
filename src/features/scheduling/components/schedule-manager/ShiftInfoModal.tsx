...
const ShiftInfoModal = () => {
  ...
  const handleDeleteEmployeeShift = async (
    shiftId: number,
    shiftName: string
  ) => {
    try {
  ...
  const isLoading =
    isLoadingScheduledShifts || isLoadingStaffShiftsGrouped || isLoadingRoles;
        ...
        <Card className="border-2 border-primary/20 bg-primary/5">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-muted-foreground">
              {openShiftsForDate.length} open shift
              {openShiftsForDate.length !== 1 ? 's' : ''} available
            </CardDescription>
          </CardHeader>
        </Card>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Are you sure you want to delete the "{shift.name}"
                              open shift for{' '}
                              {dayjs(selectedDate).format('DD/MM/YYYY')}? This
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                              onClick={() =>
                                handleDeleteOpenShift(shift.id, shift.name)
                              }
        ...
        <Card className="border-2 border-dashed border-muted">
          <CardContent className="pt-12 pb-12">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                {selectedStaffName} has no shifts on{' '}
                {dayjs(selectedDate).format('DD/MM/YYYY')}
              </p>
              <Button onClick={handleAddClick} className="gap-2 font-medium">
                <Plus className="w-4 h-4" />
      ...
      <div className="space-y-6">
        <Card className="border-2 border-primary/20 bg-primary/5">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-muted-foreground">
              {employeeShiftsForDate.length} shift
              {employeeShiftsForDate.length !== 1 ? 's' : ''} scheduled • Role:{' '}
              {getStaffRole}
            </CardDescription>
          </CardHeader>
        </Card>
        ...
        <div className="py-6">
          {isLoading ? (
            ...
            <div className="space-y-6">
              ...
              {shiftInfoModalType === 'employee-shift' &&
                renderEmployeeShiftInfo()}
            </div>
          )}
        </div>
