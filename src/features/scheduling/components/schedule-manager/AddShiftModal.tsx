...
import { Clock, Users, MapPin, AlertTriangle, CheckCircle, UserPlus, Calendar } from "lucide-react"
                ...
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader className="border-b pb-6">
                        <DialogTitle className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-foreground">Register for Shift</h1>
                ...
                <DialogHeader className="border-b pb-6">
                    <DialogTitle className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-foreground">Register for Shift</h1>
                            <p className="text-base text-muted-foreground mt-1">
                                Register {selectedStaffName} for available shifts on {dayjs(selectedDate).format("DD/MM/YYYY")}
                            </p>
                        </div>
                    </DialogTitle>
                </DialogHeader>
                    ...
                    <Card className="border-2 border-primary/20 bg-primary/5">
                        <CardHeader className="pb-4">
                            ...
                            <CardDescription className="text-muted-foreground">
                                Employee: {selectedStaffName} • Role: {staffRole} • Date: {dayjs(selectedDate).format("dddd, DD/MM/YYYY")}
                            </CardDescription>
                        </CardHeader>
                    </Card>
                            ...
                            <CardContent className="pt-12 pb-12">
                                <div className="text-center text-muted-foreground">
                                    ...
                                    <p className="text-sm text-muted-foreground mb-2">
                                        No open shifts available for role {staffRole} on {dayjs(selectedDate).format("DD/MM/YYYY")}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                        Total open shifts for this date: {scheduledShifts?.filter((s: any) =>
                        ...
                        <div className="space-y-6">
                            <Card className="border-2 border-secondary/50 bg-secondary/10">
                                <CardHeader className="pb-4">
                                    ...
                                    <CardDescription className="text-muted-foreground">
                                        {availableOpenShifts.length} open shift{availableOpenShifts.length !== 1 ? 's' : ''} available for your role
                                    </CardDescription>
                                </CardHeader>
                            </Card>
                            ...
                            <div className="grid gap-4">
                                {availableOpenShifts.map((shift: any, index: number) => {
                                    ...
                                    return (
                                        <Card key={shift.id || index} className={`border-2 transition-colors ${hasConflict
                                            ...
                                            <CardHeader className="pb-4">
                                                <div className="flex justify-between items-start">
                                                    <CardTitle className="flex items-center gap-3 text-lg font-semibold text-foreground">
                                                        <div className={`flex items-center justify-center w-8 h-8 rounded-lg ${hasConflict
                    ...
                    {existingStaffShifts.length > 0 && (
                        <Card className="border-2 border-muted bg-muted/30">
                            <CardHeader className="pb-4">
                                ...
                                <CardDescription className="text-muted-foreground">
                                    Current shifts for {dayjs(selectedDate).format("DD/MM/YYYY")}
                                </CardDescription>
                            </CardHeader>
