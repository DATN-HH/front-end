...
import { Printer, CreditCard, Banknote, CheckCircle, Clock, AlertCircle } from 'lucide-react';
...
export function BookingSuccessDialog({
    ...
    useEffect(() => {
        if (statusData?.payload?.bookingStatus === 'DEPOSIT_PAID' && !hasProcessedPayment) {
            setIsPolling(false);
    ...
    const isPaymentCompleted = statusData?.payload?.bookingStatus === 'DEPOSIT_PAID';
    const changeAmount = givenAmount ? givenAmount - bookingData.totalDeposit : 0;
                ...
                <DialogHeader>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                        ...
                        <Badge variant={isPaymentCompleted ? 'default' : 'secondary'}>
                            {statusData?.payload?.bookingStatus || bookingData.bookingStatus}
                        </Badge>
                    </div>
                </DialogHeader>
                    ...
                    <div className="space-y-4">
                        <div className="p-4 bg-gray-50 rounded-lg">
                            ...
                            <div className="space-y-2 text-sm">
                                ...
                                <div className="flex justify-between">
                                    <span>Date & Time:</span>
                                    <span>{new Date(bookingData.startTime).toLocaleString('vi-VN')}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span>Duration:</span>
                                    <span>{Math.round((new Date(bookingData.endTime).getTime() - new Date(bookingData.startTime).getTime()) / (1000 * 60 * 60))} hours</span>
                                </div>
                                <div className="flex justify-between">
                                    <span>Guests:</span>
                                ...
                                {bookingData.notes && (
                                    <div className="flex justify-between">
                                        <span>Notes:</span>
                                        <span className="text-right">{bookingData.notes}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="p-4 bg-gray-50 rounded-lg">
                            <h3 className="font-semibold mb-3">Reserved Tables</h3>
                            <div className="space-y-2">
                                {bookingData.bookedTables.map((table) => (
                                    <div key={table.tableId} className="flex justify-between items-center">
                                        <div>
                                            <span className="font-medium">{table.tableName}</span>
                                            <span className="text-sm text-gray-500 ml-2">({table.tableType})</span>
                                        </div>
                                        <span className="text-sm">{formatCurrency(table.deposit)}</span>
                                    </div>
                                ))}
                                <Separator />
                                <div className="flex justify-between font-semibold text-lg">
                                    <span>Total Deposit:</span>
                                    <span>{formatCurrency(bookingData.totalDeposit)}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {!isPaymentCompleted && (
                            <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
                                ...
                                <p className="text-sm text-orange-700">
                                    ...
                                    Booking will expire at: {new Date(bookingData.expireTime).toLocaleString('vi-VN')}
                                </p>
                            </div>
                        )}
                    </div>
                        ...
                        {!isPaymentCompleted && bookingData.qrCode && (
                            <div className="p-4 bg-blue-50 rounded-lg">
                                ...
                                <div className="text-center">
                                    <QRCode value={bookingData.qrCode} size={200} />
                        ...
                        {!isPaymentCompleted && (
                            <div className="p-4 bg-green-50 rounded-lg">
                                ...
                                {!showCashPayment ? (
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            <Label htmlFor="givenAmount">Amount Given (VND)</Label>
                                            <Input
                                                id="givenAmount"
                                                type="text"
                                                value={givenAmount}
                                                onChange={(e) => {
                                                    ...
                                                    setGivenAmount(parseInt(value) || 0);
                                                }}
                                        ...
                                        {changeAmount > 0 && (
                                            <div className="p-2 bg-yellow-100 rounded text-sm">
                                                <span className="font-medium">Change: {formatCurrency(changeAmount)}</span>
                                            </div>
                                        )}
                                        ...
                                        <div className="flex gap-2">
                                            <Button
                                                ...
                                                disabled={!givenAmount || cashPaymentMutation.isPending}
                                                className="flex-1"
                                            >
                                                {cashPaymentMutation.isPending ? 'Processing...' : 'Confirm Payment'}
                                            </Button>
                                            <Button
                                                variant="outline"
                    ...
                    <div className="receipt">
                        ...
                        {bookingData.bookedTables.map((table) => (
                            <div key={table.tableId} className="info-row">
                                <span>{table.tableName} ({table.tableType})</span>
