...
export function FeedbackDetailModal({ feedback, isOpen, onClose, onUpdate }: FeedbackDetailModalProps) {
  ...
  const handleStatusChange = async (newStatus: string) => {
    setIsUpdating(true);
    try {
      await managerFeedbackAPI.updateFeedbackStatus(feedback.id, newStatus);
  ...
  const handlePriorityChange = async (newPriority: string) => {
    setIsUpdating(true);
    try {
      await managerFeedbackAPI.updateFeedbackPriority(feedback.id, newPriority);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Feedback #{feedback.id} • {feedback.feedbackType === 'RESTAURANT' ? 'Restaurant' : 'Product'} Feedback
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {feedback.customerPhone && (
                    <p className="text-sm text-muted-foreground">{feedback.customerPhone}</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                {feedback.orderId && (
                  <div className="flex items-center gap-2 text-sm">
                    <span>Order ID: #{feedback.orderId}</span>
                    {feedback.orderType && (
                      <Badge variant="outline">{feedback.orderType}</Badge>
                    )}
                  </div>
                )}
              </div>
          ...
          {feedback.responseText && (
            <div>
              <Label className="text-sm font-medium">Current Response</Label>
              <div className="mt-2 p-4 bg-blue-50 border-l-4 border-blue-200 rounded">
                ...
                <p className="text-sm text-blue-700">{feedback.responseText}</p>
              </div>
            </div>
          )}
