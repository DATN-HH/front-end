...
const responseTemplates = [
    {
        ...
        template:
            "Thank you so much for your wonderful feedback! We're thrilled to hear about your positive experience. We look forward to serving you again soon!",
    },
    {
        id: 'apologize_service',
        title: 'Apologize (Service)',
        template:
            'We sincerely apologize for the service issues you experienced. This does not meet our standards, and we are taking immediate action to improve. Please give us another chance to serve you better.',
    },
    {
        id: 'apologize_food',
        title: 'Apologize (Food Quality)',
        template:
            "We apologize for the food quality issues. Your feedback is valuable to us, and we are working with our kitchen team to ensure this doesn't happen again. We'd love to invite you back for a better experience.",
    },
    {
        id: 'follow_up',
        title: 'Follow Up',
        template:
            'Thank you for your feedback. We have reviewed your concerns with our team and implemented improvements. Please feel free to contact us directly if you have any other concerns.',
    },
];
...
export function ManagerFeedbackDashboard({
    ...
    const filteredFeedback = feedbackData.filter((feedback) => {
        const matchesStatus =
            filters.status === 'ALL' ||
            feedback.feedbackStatus === filters.status;
        const matchesPriority =
            filters.priority === 'ALL' ||
            feedback.priority === filters.priority;
        const matchesBranch =
            filters.branch === 'ALL' ||
            feedback.branchId.toString() === filters.branch;
        const matchesSearch =
            filters.search === '' ||
            feedback.title
                .toLowerCase()
        ...
        return (
            matchesStatus && matchesPriority && matchesBranch && matchesSearch
        );
    });
    ...
    const stats = {
        ...
        responded: feedbackData.filter((f) => f.feedbackStatus === 'RESPONDED')
            .length,
        urgent: feedbackData.filter(
            (f) => f.priority === 'URGENT' && f.feedbackStatus === 'PENDING'
        ).length,
        avgRating:
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between">
                    <div className="text-sm text-gray-700">
                        ...
                        of {sortedFeedback.length} results
                    </div>
