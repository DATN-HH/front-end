...
export function ProductAttributeEditModal({ open, onOpenChange, attributeId }: ProductAttributeEditModalProps) {
    ...
    const onSubmit = async (data: FormData) => {
        try {
            ...
            await updateAttributeMutation.mutateAsync({ id: attributeId, data: requestData });
    ...
    if (!attribute) {
        return (
            <Dialog open={open} onOpenChange={handleClose}>
                <DialogContent className="sm:max-w-[600px]">
                    <div className="flex items-center justify-center p-6">
                        <div className="text-center text-red-500">Failed to load attribute data</div>
                    </div>
                </DialogContent>
            </Dialog>
        );
