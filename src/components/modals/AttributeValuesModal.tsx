...
export function AttributeValuesModal({
    ...
    useEffect(() => {
        if (open && externalEditingValueId && values.length > 0) {
            const valueToEdit = values.find(
                (v) => v.id === externalEditingValueId
            );
            if (valueToEdit) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[900px] max-h-[80vh] overflow-y-auto">
                ...
                <Tabs
                    defaultValue={editingValueId ? 'form' : 'values'}
                    key={editingValueId || 'default'}
                    className="w-full"
                >
                    ...
                    <TabsContent value="values" className="space-y-4">
                        ...
                        {isLoading ? (
                            ...
                            <div className="text-center py-8 text-muted-foreground">
                                No values found. Create the first value to get
                                started.
                            </div>
                        ) : (
