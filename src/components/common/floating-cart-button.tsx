...
export function FloatingCartButton() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <Button onClick={() => router.push("/cart")} className="rounded-full h-14 w-14 shadow-lg relative" size="lg">
        <ShoppingCart className="h-6 w-6" />
        <Badge className="absolute -top-2 -right-2 h-6 w-6 rounded-full p-0 text-xs">{totalItems}</Badge>
      </Button>
