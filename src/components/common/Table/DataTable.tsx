...
import {
    extractPinningFromColumns,
    combineColumnPinning,
...
export function DataTable<TData, TValue>({
        ...
        if (typeof window !== 'undefined' && tableId) {
            try {
                ...
                if (savedState) {
                    ...
                    setColumnVisibility(
                        parsedState[TABLE_STATE_KEYS.COLUMN_VISIBILITY] || {}
                    );
                    setColumnOrder(
                        parsedState[TABLE_STATE_KEYS.COLUMN_ORDER] || []
                    );
                    // Use saved pinning if it exists and has content, otherwise keep initial pinning
                    if (
                        parsedState[TABLE_STATE_KEYS.COLUMN_PINNING] &&
                        (parsedState[TABLE_STATE_KEYS.COLUMN_PINNING].left
                            ?.length > 0 ||
                            parsedState[TABLE_STATE_KEYS.COLUMN_PINNING].right
                                ?.length > 0)
                    ) {
                        setColumnPinning(
                            parsedState[TABLE_STATE_KEYS.COLUMN_PINNING]
    ...
    useEffect(() => {
        if (columnOrder.length === 0) {
            const initialColumnOrder = table
                .getAllColumns()
    ...
    const handleToggleAllColumns = () => {
        ...
        const allVisible = hidableColumns.every((column) =>
            column.getIsVisible()
        );
    ...
    const getSelectAllState = () => {
        const hidableColumns = table
            .getAllColumns()
            .filter((column) => column.getCanHide());
        const visibleCount = hidableColumns.filter((column) =>
            column.getIsVisible()
        ).length;
    ...
    return (
        ...
        >
            <div className="w-full">
                ...
                {/* Table Section */}
                <div
                    className={`border ${!isMobile && (enableSearch || enableFiltering || enableColumnVisibility || enableExport) ? 'border-t-0' : ''} border-border ${!isMobile && (enableSearch || enableFiltering || enableColumnVisibility || enableExport) ? 'rounded-b-xl' : 'rounded-xl'} bg-background shadow-sm`}
                >
                    <div
                ...
                {enablePagination && (
                    <div
                        className={
                            'bg-background border border-t-0 border-border rounded-b-xl px-4 lg:px-6 py-3 lg:py-4'
                        }
                    >
