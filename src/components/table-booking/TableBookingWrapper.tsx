...
import { useCreateBooking, useCreateAdminBooking, CreateBookingRequest, CreateBookingResponse } from "@/api/v1/table-booking"
import { LocationSelector } from "@/app/(for-guest)/table-booking/components/LocationSelector"
import { DateTimeSelector } from "@/app/(for-guest)/table-booking/components/DateTimeSelector"
import { BookingForm } from "@/app/(for-guest)/table-booking/components/BookingForm"
import { MultiSelectFloorCanvas } from "@/app/(for-guest)/table-booking/components/MultiSelectFloorCanvas"
import { BookingConfirmDialog } from "@/app/(for-guest)/table-booking/components/BookingConfirmDialog"
...
export default function TableBookingWrapper({
    ...
    const [selectedBranch, setSelectedBranch] = useState<number | null>(initialBranchId || null)
    ...
    const { data: floors = [], isLoading: floorsLoading } = useFloorsByBranch(selectedBranch || 0, !!selectedBranch)
    ...
    const { data: tablesStatus = [], isLoading: isLoadingTableStatus } = useFloorTablesStatus(
        selectedFloor || 0,
        selectedDateTime,
    ...
    const availableTables = useMemo(() => {
        ...
        if (!tablesStatus?.payload?.availableTablesList || !Array.isArray(tablesStatus.payload.availableTablesList)) {
            return []
        }
    ...
    const handleBookingDataChange = useCallback((data: Partial<typeof bookingData>) => {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        if (mode === 'admin') {
            ...
            const adminBookingPayload = {
                ...
                tableIds: Array.isArray(selectedTables) ? selectedTables.map(t => t.id) : [],
            ...
            try {
                const response = await createAdminBookingMutation.mutateAsync(adminBookingPayload)
        ...
        const bookingPayload = {
            ...
            guests: bookingData.guests,
            notes: bookingData.notes,
            tableId: Array.isArray(selectedTables) ? selectedTables.map(t => t.id) : [],
            customerName: bookingData.customerName.trim(),
            customerPhone: bookingData.customerPhone.trim(),
        ...
        try {
            ...
            if (response.success && response.payload) {
                ...
                showError("Error", response.error?.message || "Failed to create booking")
            }
        } catch (error: any) {
        ...
        selectedTables, bookingData, selectedDateTime, selectedBranch, selectedFloor, duration,
        mode, onBookingComplete, createBookingMutation, createAdminBookingMutation, success, showError
    ])
    ...
    return (
        <div className={mode === 'admin' ? 'p-4' : 'min-h-screen bg-gray-50 p-4'}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-6">
                        ...
                        <LocationSelector
                            ...
                            onBranchChange={(branchId) => setSelectedBranch(parseInt(branchId))}
                            onFloorChange={(floorId) => setSelectedFloor(parseInt(floorId))}
                            branchesLoading={branchesLoading}
                            floorsLoading={floorsLoading}
                            disableBranch={mode === 'admin' && !!initialBranchId}
                        />
                            ...
                            <Card>
                                <CardHeader className="pb-2">
                                    ...
                                    <CardDescription className="text-sm">
                                        Choose your preferred tables. Selected: {selectedTables.length}
                                    </CardDescription>
                                </CardHeader>
                                <CardContent>
                                    {currentFloor ? (
                                        <MultiSelectFloorCanvas
                                            floor={{
                                                ...
                                                imageUrl: currentFloor.imageUrl || '',
                                                order: currentFloor.order || 0,
                                                status: currentFloor.status || 'ACTIVE',
                                                createdAt: currentFloor.createdAt || '',
                                                updatedAt: currentFloor.updatedAt || ''
                                            }}
                                            tables={Array.isArray(tables) ? tables : []}
                                            selectedTables={selectedTables}
                                            onTableSelect={handleTableSelect}
                                            selectableTables={selectableTableIds}
                                        />
                                    ) : (
                                        <div className="text-center text-muted-foreground p-8">
                                            Floor data not available. Please refresh the page.
                                        </div>
                                    )}
                                </CardContent>
                            </Card>
                    ...
                    <div className="space-y-6">
                        <BookingForm
                            ...
                            isSubmitting={mode === 'admin' ? createAdminBookingMutation.isPending : createBookingMutation.isPending}
                            mode={mode}
                        />
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-3">
                                    ...
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span>{selectedTables.reduce((sum, t) => sum + t.capacity, 0)} guests</span>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-1 pt-2">
                                        {Array.isArray(selectedTables) && selectedTables.map(table => (
                                            <Badge key={table.id} variant="secondary" className="text-xs">
                                                {table.tableName} ({table.tableType?.name})
                                            </Badge>
                                        ))}
                                    </div>
                                </CardContent>
